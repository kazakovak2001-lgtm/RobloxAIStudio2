...
- **Replacement semantics** — screens are keyed by screen name under `ReplicatedStorage/AIStudioArtifacts/UI_GENERATION/`, replaced destructively on re-delivery, with removed screens swept only when they carry the `AIStudioManaged` attribute so hand-added siblings survive.
- **Receipt verification** — make `instancePath` required and verified against a backend-computed expected path in `StudioRuntime.verifyImportEvidence`, and promote the already-returned-but-discarded `instancePaths` array to the receipt so the named screens can be proven to have landed at the named paths.
...
- **Verification** — missing `instancePath`, a wrong path, and a missing or extra screen entry must each be rejected; a correct receipt must verify.
