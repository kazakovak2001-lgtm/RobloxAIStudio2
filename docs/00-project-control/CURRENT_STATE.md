...
- August 8, 2026: PROVIDER-1A implemented on branch `claude/robloxaistudio2-audit-ebc9c8`. It is not merged, so the runtime pair recorded above is unchanged. A repository audit found three defects that composed into one truthfulness failure: `LLMProviderFactory.createByName()` handled only `openai`, `anthropic`, `gemini` and `groq`, so `DEFAULT_PROVIDER=ollama` or `openrouter` fell through and silently returned `provider: null`; every agent guards its own LLM call and returns a deterministic fallback when none is wired; and `LuaGeneratorAgent`'s fallback is a complete hardcoded game deliberately written to satisfy `getPlayableLuaIssues()`. A misconfigured deployment therefore produced a green pipeline, a Studio import with valid receipts and a positive playtest score for a game unrelated to the user's prompt, with nothing durable to distinguish it from a real generation. The slice adds explicit `ollama` and `openrouter` selection, rejects unknown provider names by name, marks an unsatisfiable explicit request with `requested`/`unsatisfied`/`unsatisfiedReason` instead of degrading silently, adds the default-off `REQUIRE_LLM_PROVIDER=true` startup refusal, and records optional `ai_mode`/`ai_provider`/`ai_model` on every `GenerationExecution`. Provenance fails safe: an execution is `ai` only when a provider resolved and no stage returned canned content; pre-existing rows read back `undefined` and are reported as unknown, never as `ai`. Storage is the existing JSONB `kv_store`, so no migration is required. Only provider and model names are recorded — never keys, endpoints or prompt bodies. Verification of the forced path initially appeared to fail with a "no API key found" startup line; that output came from a checkout without the fix, and forced Ollama was confirmed keyless on the fixed branch, logging `Ollama (forced, url: http://localhost:11434, model: qwen2.5-coder:7b)`. Two regressions now pin that behavior explicitly: one deletes every provider credential and asserts forced Ollama still resolves with a startup line that never mentions an API key, and one walks `openai`, `anthropic`, `gemini`, `groq` and `openrouter` asserting each still fails closed without its credential and resolves with it, so the keyless path cannot be read as globally relaxed credential validation. Evidence limits, stated plainly: verified by typecheck, lint, architecture and boundary validators, 1051 passing backend tests, 30 provider-selection tests and 5 provenance tests; the one failing suite (`persistence.test.ts` DB health) asserts an unreachable database and fails only because the audit machine had a live PostgreSQL, and imports nothing this slice changed. No Studio-attached live run of a provider-backed generation was performed for this slice. The auto-detection path still defaults to the hardcoded `llama3` model; that is recorded as remaining debt rather than silently changed.
