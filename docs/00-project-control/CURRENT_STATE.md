...
- July 28, 2026: CLEANUP-1C physical removal implemented under issue #37 from exact baseline `85a2fa8d512738e6d02ffae42da77af7a27db6fc` — 168 root `src/` files, five frontend entry/config files, and three archival combined-deployment files are deleted (176 paths total). Five runtime and seven development-only direct package declarations are pruned; `socket.io-client` remains because the active composed-release verifier consumes it. The existing production-readiness audit now checks the canonical Studio plugin instead of removed `src/shared`. The evolved audit requires the exact `D`/`M`/`A` diff, synchronized root lockfile declarations, zero remaining consumers, active-release isolation, and the protected Merge Gate dependency. Local typecheck, architecture, boundaries, lint, formatting, 666 tests, repository validation, build, and a negative root-`src/` reintroduction control passed. Protected CI and merge evidence are pending; CLEANUP-1D remains blocked. See `docs/project/CLEANUP-1C_PHYSICAL_REMOVAL.md`.
