...
7. writes a JSON result, Markdown report, and raw Studio output under `artifacts/studio-acceptance/latest/`.
...
Acceptance fixtures use a per-run GUID namespace and clean up only instances they created. No runtime handle is persisted. Generated reports are ignored local evidence and do not become release truth merely by existing.
