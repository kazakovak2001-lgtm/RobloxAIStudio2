...
| `studio_type_text`        | Verifies a visibly focused screenshot target, then types up to 2,000 plain characters. | Ask every call         |
| `studio_press_key`        | Verifies a visibly focused target, then sends one allowlisted navigation or test key.  | Ask every call         |
| `studio_capture_evidence` | Persists the exact approved single-use PNG under ignored local evidence.               | Ask every call         |

Claude Code permissions deliberately do not blanket-approve input or evidence persistence. Do not change the `ask` rules to `allow`. Each screenshot returns a single-use `captureId`. Before input, the bridge first recaptures Studio without activating it and rejects the action unless the PID, native HWND, process start time, title, executable, minimized state, bounds, dimensions, UI chrome, and approved 5×5 target neighborhood still match. HWND and process start time are checked again inside the final input process both before and after focus, preventing a recreated window or reused PID from inheriting approval. The target comparison permits only a bounded uniform luminance shift in either direction so the active screenshot can safely become inactive while the operator approves the action; large recolors and structural changes are rejected. The bridge then activates and recaptures Studio with the same bounded comparison. The Windows input helper receives this active fingerprint and repeats the strict local check in the same process immediately before input. A click uses one atomic absolute `SendInput` sequence at the verified virtual-desktop coordinates, so intervening cursor movement cannot retarget it. The UI-chrome fingerprint uses bounded perceptual tolerance while excluding the dynamic 3D viewport, title-bar foreground styling, and live Output area. Text and contextual key entry never click: first use a separately approved `studio_click` to focus the intended control, capture a new screenshot that visibly shows its focused state, and then approve the text or key action against that new single-use capture. This prevents one approved operation from both changing UI focus and sending follow-on input to the changed UI. `SHIFT_F5` is the sole focus-click and target-fingerprint exception because it only stops a running Play test whose viewport is expected to change; native identity, title, executable, non-minimized state, bounds, layout, foreground, and approval checks still apply. Double-click is not exposed; every permitted click is one approved left click. Evidence capture consumes a `captureId`, writes the exact PNG bytes that the operator reviewed, and refuses to overwrite an existing run/name path; it never takes a later screenshot after approval. The MCP tool still cannot semantically identify every dangerous button, so the operator approval prompt, capture binding, PID pinning, and the `studio-operator` policy are all load-bearing.
