...
| `studio_type_text`        | Verifies and clicks a screenshot point, then types up to 2,000 plain characters.          | Ask every call         |
| `studio_press_key`        | Verifies and focuses a screenshot target, then sends one allowlisted navigation/test key. | Ask every call         |
| `studio_capture_evidence` | Persists an approved Studio-window PNG under ignored local evidence.                      | Ask every call         |

Claude Code permissions deliberately do not blanket-approve input or evidence persistence. Do not change the `ask` rules to `allow`. Each screenshot returns a single-use `captureId`. Before input, the bridge first recaptures Studio without activating it and rejects the action unless the PID, title, executable, minimized state, bounds, dimensions, UI chrome, and approved 5×5 target neighborhood still match. It then activates and recaptures Studio, accepting only a bounded uniform luminance shift in that target neighborhood so ordinary active/inactive selection styling is tolerated while large recolors and structural changes are rejected. The Windows input helper receives this active fingerprint and repeats the local check in the same process immediately before cursor movement and its click. It also checks that no separate window or popup covers the target both before and after moving the cursor. The UI-chrome fingerprint uses bounded perceptual tolerance while excluding the dynamic 3D viewport, title-bar foreground styling, and live Output area. Text and contextual key entry explicitly click the verified target, wait for the focus transition, and revalidate the same target fingerprint and window ownership immediately before sending characters or the key. A focus-style change that exceeds the strict final threshold safely refuses input; capture a new screenshot and retry against the now-focused control. `SHIFT_F5` is the sole click-free and target-fingerprint-free key because it only stops a running Play test whose viewport is expected to change; PID, handle, title, executable, non-minimized state, bounds, layout, foreground, and approval checks still apply. Double-click is not exposed; every permitted click is one approved left click. Evidence capture refuses to overwrite an existing run/name path. The MCP tool still cannot semantically identify every dangerous button, so the operator approval prompt, capture binding, PID pinning, and the `studio-operator` policy are all load-bearing.
