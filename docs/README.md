...
- `WORLD-1B`: complete — backend PR #209 plus the [deferred Studio acceptance result](./00-project-control/STUDIO-DEFERRED-ACCEPTANCE_RESULT.md) and changed-design follow-up in PR #239. Real Studio materialization, semantic attributes, repeat export, creator preservation, fail-closed collision, Play separation, and managed sweep from five entities to four passed. The acceptance prerequisite for `WORLD-1C` is satisfied; `WORLD-1C` remains separately scoped and unimplemented.
