  ...
  useEffect(() => {
    ...
    const onStepCompleted = (payload: GenerationProgressEvent) => {
      setState((prev) => {
        ...
        // Extract cost data if present in payload
        const costData = (payload as unknown as Record<string, unknown>)
          .cost as { tokens?: number; cost?: number } | undefined;
        ...
        const updatedAgents: AgentState[] = existingAgent
          ? prev.agents.map((item) =>
              item.id === payload.agentId
                ? {
                    ...
                    ...(costData?.tokens !== undefined && {
                      tokens: costData.tokens,
                    }),
                    ...(costData?.cost !== undefined && {
                      cost: costData.cost,
                    }),
                  }
                : item,
          ...
          : [
              ...prev.agents,
              {
                ...
                ...(costData?.cost !== undefined && { cost: costData.cost }),
              },
            ];
    ...
    socket.on("step.failed", onStepFailed as never);
    ...
    return () => {
      ...
      socket.off("step.failed", onStepFailed as never);
