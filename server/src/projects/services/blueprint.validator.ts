...
export class BlueprintValidator {
  validate(blueprint: GameBlueprint): ValidationResult {
    ...
    if (!blueprint.genre || blueprint.genre.length === 0) errors.push("At least one genre is required");
    if (!blueprint.user_id) errors.push("User ID is required");
    if (!blueprint.difficulty) errors.push("Difficulty is required");
    if (!blueprint.estimated_players) errors.push("Estimated players is required");

    if (blueprint.gameplay) {
      if (!blueprint.gameplay.mechanics || blueprint.gameplay.mechanics.length === 0) {
    ...
    if (!blueprint.architecture || !blueprint.architecture.client_architecture) {
      warnings.push("Architecture is incomplete");
    }

    if (!blueprint.code_spec || !blueprint.code_spec.modules || blueprint.code_spec.modules.length === 0) {
      warnings.push("No code modules defined");
    }

    if (!blueprint.assets || (!blueprint.assets.models?.length && !blueprint.assets.textures?.length && !blueprint.assets.sounds?.length)) {
  ...
  validateForGeneration(blueprint: GameBlueprint): ValidationResult {
    ...
    if (blueprint.status !== "draft" && blueprint.status !== "validated") {
      errors.push(`Cannot generate: blueprint status is ${blueprint.status}, expected draft or validated`);
    }
  ...
  validateForExport(blueprint: GameBlueprint): ValidationResult {
    ...
    if (blueprint.status !== "ready_for_export" && blueprint.status !== "exported") {
      errors.push(`Cannot export: blueprint status is ${blueprint.status}, expected ready_for_export or exported`);
    }
