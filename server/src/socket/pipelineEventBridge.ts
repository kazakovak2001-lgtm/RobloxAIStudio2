  ...
  events.onEvent(async (evt) => {
    ...
    switch (evt.type) {
      ...
      case "pipeline.completed": {
        ...
        // GEN-LIFECYCLE-SPLIT-001. This fires when the agent DAG finishes,
        // which is before the artifacts are recorded, before validation and
        // before the canonical package is committed. It used to mark the
        // project `ready` here, with a hardcoded quality score of 100, so a run
        // whose recorder or validation then failed left the project claiming a
        // finished, perfect generation while its durable execution said failed
        // and no package existed.
        //
        // The terminal transition belongs to GenerationOutcomeCoordinator,
        // which runs after all of that and derives the score from the steps
        // that actually ran. Nothing durable is written here any more: this
        // event now means only that the DAG finished.
        console.log(
          "[pipeline-bridge] forwarding",
          "pipeline.completed",
          payload,
