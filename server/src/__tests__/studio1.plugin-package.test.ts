...
describe("STUDIO-1e deterministic Roblox plugin package", () => {
  it("packages only the canonical active source hierarchy as an installable model", async () => {
    ...
    expect(result.bundlePath).toMatch(/RobloxAIStudioPlugin-v1\.11\.1\.rbxmx$/);
    expect(manifest).toMatchObject({
      ...
      bundleFile: "RobloxAIStudioPlugin-v1.11.1.rbxmx",
    });
    ...
    expect(checksums).toContain(
      `${sha256(manifestContent)}  RobloxAIStudioPlugin-v1.11.1.manifest.json`,
    );
  });
