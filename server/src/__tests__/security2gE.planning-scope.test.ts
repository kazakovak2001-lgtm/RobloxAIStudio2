...
describe("SECURITY-2G-E planning scope", () => {
  it("requires project access before creating plans", () => {
    ...
    expect(routeSource.indexOf("await access.requireProjectAccess(req, res, projectId)")).toBeLessThan(
      routeSource.indexOf("planner.createPlan(goal)"),
    );
  });
  ...
  it("resolves stored plan project ownership before execute and read", () => {
    ...
    expect(routeSource.indexOf("requirePlanProjectAccess(req, res, plan)")).toBeLessThan(
      routeSource.indexOf("executor.executePlan("),
    );
  });

  it("wires the shared project access control into the planning router", () => {
    expect(indexSource).toContain("createPlanningRouter(agentRegistry, access)");
  });
});
