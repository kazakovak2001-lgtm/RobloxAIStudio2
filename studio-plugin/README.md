...
- **Materialization is atomic.** The whole tree is validated, then built detached with no parent, and only attached once every instance exists. A failure therefore leaves nothing behind — which matters because `SyncManager` reports a failed command _without_ rolling back instances already created.
- **Ownership is respected.** Only instances carrying the `AIStudioManaged` attribute are ever destroyed. A generated screen name colliding with a hand-built instance fails the export instead of deleting the creator's work, and hand-added children of a managed screen are carried across a replacement.
