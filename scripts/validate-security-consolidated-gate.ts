...
function validateExceptions(
  ...
  for (const exception of exceptions) {
    if (!exact(exception.owner))
      errors.push("exception owner missing or wildcard");
    if (!exact(exception.approvalReference))
      errors.push("exception approval missing");
    if (!exact(exception.rationale)) errors.push("exception rationale missing");
    if (!exact(exception.compensatingControl))
      errors.push("exception control missing");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(exception.expiresAt ?? "")))
      errors.push("exception expiry missing");
    else if (String(exception.expiresAt) < today)
      errors.push("exception expired");
    if (
      Object.keys(exception).some((key) =>
        ["secret", "plaintext", "credential"].includes(key),
      )
    )
...
async function validateRepository(contract: ConsolidatedContract) {
  ...
  const release = await readJson<{
    frontendRelease?: { repository?: string; commit?: string };
  ...
  const lockfile = await readFile(
    path.join(root, contract.identityInputs.lockfile),
  );
  const matrix = await readFile(
    path.join(root, contract.identityInputs.authorizationMatrix),
  );
  const matrixJson = JSON.parse(matrix.toString("utf8")) as {
    controlId?: string;
    operations?: unknown[];
  };
  ...
  if (
    release.frontendRelease?.repository !== contract.frontendSource.repository
  )
    errors.push("paired Frontend repository does not match release inventory");
  if (!shaPattern.test(frontendSha))
    errors.push("paired Frontend commit is not an exact SHA");
  if (matrixJson.controlId !== "SECURITY-2G-E")
    errors.push("authorization matrix control mismatch");
  if (
    !Array.isArray(matrixJson.operations) ||
    matrixJson.operations.length === 0
  )
  ...
  for (const control of contract.requiredControls) {
    ...
    if (
      !workflow.includes(`name: ${control.job}`) &&
      !workflow.includes(`name: \"${control.job}\"`)
    )
  ...
  const today =
    process.env.SECURITY_2G_F_TODAY ?? new Date().toISOString().slice(0, 10);
  errors.push(...validateExceptions(packageJson.securityPolicy ?? {}, today));

  const backendSha =
    process.env.GITHUB_SHA ??
    process.env.SECURITY_2G_F_BACKEND_SHA ??
    "0".repeat(40);
  if (!shaPattern.test(backendSha))
    errors.push("backend source is not an exact SHA");
  ...
  return {
    ...
    evidence: {
      ...
      requiredControls: contract.requiredControls.map(
        ({ id, owner, evidence, job, bindings }) => ({
          id,
          owner,
          evidence,
          job,
      ...
      exceptionCount:
        (((packageJson.securityPolicy?.scannerExceptions as unknown[]) ?? [])
          .length ?? 0) +
        (((packageJson.securityPolicy?.exceptions as unknown[]) ?? []).length ??
          0),
...
async function runFixtures(contract: ConsolidatedContract) {
  assert.equal(
    validateContract(contract).length,
    0,
    "positive contract fixture failed",
  );
  const invalidContracts: ConsolidatedContract[] = [
    { ...contract, requiredControls: contract.requiredControls.slice(1) },
    {
      ...contract,
      requiredCheckRuns: contract.requiredCheckRuns.filter(
        (x) => x !== "CI Pipeline",
      ),
    },
    {
    ...
    {
      ...contract,
      exceptionPolicy: { ...contract.exceptionPolicy, expiryRequired: false },
    },
  ];
  for (const fixture of invalidContracts) {
    assert.notEqual(
      validateContract(fixture).length,
      0,
      "negative contract fixture passed",
    );
  }
  ...
  assert.notEqual(
    validateExceptions({ scannerExceptions: [invalidException] }, "2026-08-02")
      .length,
    0,
  );
}
