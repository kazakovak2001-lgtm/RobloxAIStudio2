...
4. request the built-in Claude Code approval for every click, typed text, key press, and evidence capture; before text or a contextual key, use a separately approved click to focus the target, capture a new screenshot that visibly shows that focus, then request the text/key action against the new capture; `SHIFT_F5` is the sole focus-click exception;
