...
Persist only specifically approved screenshots with `studio_capture_evidence`, passing the same single-use `captureId` returned with the image being approved. The tool saves those exact PNG bytes and does not take a later screenshot. Use a lowercase run label and descriptive evidence name, and never reuse a run/name pair because existing evidence cannot be overwritten. Keep generated evidence under the ignored `artifacts/studio-acceptance/operator/` boundary. Do not commit evidence automatically.
